...
bool NavConfig::enumProfileCallback(std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& res)
{
  ...
  for (Json::Int i = 0; i <= size; i++)
  {
    ...
    enum_src["navx_profiles"].append(item);
  }
...
void NavConfig::setupProfileDefault(size_t index)
{
  ...
  const agv05_navx::NavConfig& cfg_default = agv05_navx::NavConfig::__getDefault__();
...
#define LOAD_PARAM(v, p, d) nh.param(p, param, d); v = param
  ...
  LOAD_PARAM(defaults_[index].profile.path_follow_p, "path_follow_p", cfg_default.path_follow_p1);
  LOAD_PARAM(defaults_[index].profile.path_follow_i, "path_follow_i", cfg_default.path_follow_i1);
  LOAD_PARAM(defaults_[index].profile.path_follow_d, "path_follow_d", cfg_default.path_follow_d1);
  LOAD_PARAM(defaults_[index].profile.path_follow_ahead_distance, "path_follow_ahead_distance", cfg_default.path_follow_ahead_distance1);
  LOAD_PARAM(defaults_[index].profile.path_follow_heading_error_max, "path_follow_heading_error_max", cfg_default.path_follow_heading_error_max1);
  nh.param("path_follow_pid_traditional", defaults_[index].profile.path_follow_pid_traditional, cfg_default.path_follow_pid_traditional1);
  ...
  LOAD_PARAM(defaults_[index].stop.straight.decel, "straight_reaching_dec", cfg_default.straight_reaching_dec);
  LOAD_PARAM(defaults_[index].stop.straight.speed, "straight_reaching_speed", cfg_default.straight_reaching_speed);
  LOAD_PARAM(defaults_[index].stop.straight.speed_min, "straight_reaching_min_speed", cfg_default.straight_reaching_min_speed);
  LOAD_PARAM(defaults_[index].stop.straight.distance, "straight_reaching_distance", cfg_default.straight_reaching_distance);
  LOAD_PARAM(defaults_[index].stop.forward.junction, "forward_stopping_distance_line_sensed", cfg_default.forward_stopping_distance_line_sensed);
  LOAD_PARAM(defaults_[index].stop.reverse.junction, "reverse_stopping_distance_line_sensed", cfg_default.reverse_stopping_distance_line_sensed);
  LOAD_PARAM(defaults_[index].stop.straight.tolerance, "localization_tolerance_distance", cfg_default.localization_tolerance_distance);
  ...
  LOAD_PARAM(defaults_[index].turn.align.speed, "turn_reaching_speed", cfg_default.turn_reaching_speed);
  LOAD_PARAM(defaults_[index].turn.align.distance, "turn_reaching_distance", cfg_default.turn_reaching_distance);
...
NavConfig& NavConfig::operator=(agv05_navx::NavConfig& config)
{
...
#define RESET_PROFILE(i) \
  ...
  { \
    ...
    \
    config.path_follow_p##i = ROUND(defaults_[index].profile.path_follow_p); \
    config.path_follow_i##i = ROUND(defaults_[index].profile.path_follow_i); \
    config.path_follow_d##i = ROUND(defaults_[index].profile.path_follow_d); \
    config.path_follow_ahead_distance##i = ROUND(defaults_[index].profile.path_follow_ahead_distance); \
    config.path_follow_heading_error_max##i = ROUND(defaults_[index].profile.path_follow_heading_error_max); \
    config.path_follow_pid_traditional##i = defaults_[index].profile.path_follow_pid_traditional; \
    \
...
#undef RESET_PROFILE
  ...
  if (config.reset_profile_stop_)
  {
    ...
    config.straight_reaching_dec = ROUND(defaults_[index].stop.straight.decel);
    config.straight_reaching_speed = ROUND(defaults_[index].stop.straight.speed);
    config.straight_reaching_min_speed = ROUND(defaults_[index].stop.straight.speed_min);
    config.straight_reaching_distance = ROUND(defaults_[index].stop.straight.distance);
    config.forward_stopping_distance_line_sensed = ROUND(defaults_[index].stop.forward.junction);
    config.reverse_stopping_distance_line_sensed = ROUND(defaults_[index].stop.reverse.junction);
    config.localization_tolerance_distance = ROUND(defaults_[index].stop.straight.tolerance);
  ...
  if (config.reset_profile_turn_)
  {
    ...
    config.turn_reaching_speed = ROUND(defaults_[index].turn.align.speed);
    config.turn_reaching_distance = ROUND(defaults_[index].turn.align.distance);
...
#undef ROUND
  ...
  agv05_navx::NavConfig::operator=(config);

#define LOAD_PROFILE(i) \
  profiles_[(i)-1].path_follow_p = path_follow_p##i; \
  profiles_[(i)-1].path_follow_i = path_follow_i##i; \
  profiles_[(i)-1].path_follow_d = path_follow_d##i; \
  profiles_[(i)-1].path_follow_ahead_distance = path_follow_ahead_distance##i; \
  profiles_[(i)-1].path_follow_heading_error_max = path_follow_heading_error_max##i; \
  profiles_[(i)-1].path_follow_pid_traditional = path_follow_pid_traditional##i; \
  \
