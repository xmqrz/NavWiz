...
ActionManualControl::ActionManualControl(Nav& nav, const Goal& goal) :
  ...
  speed_limit_(goal.speed > 0 ? goal.speed : 0),
  force_enable_sensor_(goal.enable_sensor),
...
void ActionManualControl::process(float frequency)
{
  ...
  bool enable_sensor = speed_limit_ > 0.0f ? force_enable_sensor_ :
                       *reinterpret_cast<uint64_t*>(&v.linear.z) != 1LL || base_.getManualCmdVelTimeout() >= 0.5;

  safety_.publishMuteBumper(!enable_sensor && speed_limit_ == 0.0f);

  // safety check
  uint8_t status = ActionProcessor::checkSafetyStatus(enable_sensor, speed_limit_ == 0.0f);
  safety_.publishNavTrigger(status != Feedback::STATUS_NORMAL, safety_internal_message_);

  // set speed limits
  speed_x_.setAccelerationLimits(config_.manual_control_acc,
                                 config_.manual_control_dec,
                                 config_.manual_control_jerk,
                                 config_.manual_control_jerk);
  speed_y_.setAccelerationLimits(config_.manual_control_acc,
                                 config_.manual_control_dec,
                                 config_.manual_control_jerk,
                                 config_.manual_control_jerk);
  speed_z_.setAccelerationLimits(config_.manual_control_acc,
                                 config_.manual_control_dec,
                                 config_.manual_control_jerk,
                                 config_.manual_control_jerk);
  ...
  {
    speed_x_.addSpeedLimit(speed_limit_);
    speed_y_.addSpeedLimit(speed_limit_);
    speed_z_.addSpeedLimit(speed_limit_ * 2.0f);
  }
  ...
  else
  {
    ...
    if (stop)
    {
      ...
      speed_z_.setSpeed(0.0f);
      base_.stop();
      if (goal_.nav != Goal::NAV_MANUAL_CONTROL)
      {
      ...
      else if (isAborted() || (!isPaused() && goal_.speed < 0))
      {
        safety_.publishMuteBumper(false);
