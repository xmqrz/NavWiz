...
void LineExtraction::mergeLines()
{
  ...
  for (std::size_t i = 0; i < lines_.size(); ++i)
  {
    ...
    Line& line_1 = lines_[i];
    Line& line_2 = i == lines_.size() - 1 ? merged_lines[0] : lines_[i + 1];

    // Get L, P_1, P_2 of consecutive lines
    Eigen::Vector2d L_1(line_1.getRadius(), line_1.getAngle());
    Eigen::Vector2d L_2(line_2.getRadius(), line_2.getAngle());
    Eigen::Matrix2d P_1;
    P_1 << line_1.getCovariance()[0], line_1.getCovariance()[1],
        line_1.getCovariance()[2], line_1.getCovariance()[3];
    Eigen::Matrix2d P_2;
    P_2 << line_2.getCovariance()[0], line_2.getCovariance()[1],
        line_2.getCovariance()[2], line_2.getCovariance()[3];
    ...
    if (chiSquared(L_1 - L_2, P_1, P_2) < 3)
    {
      ...
      cov[3] = P_m(1, 1);
      std::vector<unsigned int> indices;
      const std::vector<unsigned int> &ind_1 = line_1.getIndices();
      const std::vector<unsigned int> &ind_2 = line_2.getIndices();
      ...
      Line merged_line(L_m[1], L_m[0], cov, line_1.getStart(), line_2.getEnd(), indices);
