...
typedef enum
{
  ...
  LASER_MODEL_LIKELIHOOD_FIELD_REFLECTORS,
...
class AMCLLaserData : public AMCLSensorData
{
  ...
  // Observation using reflectors
  public: std::vector<int> reflectors;
};
...
{
  // Default constructor
  public: AMCLLaser(size_t max_beams, map_t* map, double reflector_radius,
                    const std::vector<pf_vector_t>* landmarks);
  ...
  public: void SetModelLikelihoodFieldReflectors(double z_hit,
             double z_rand,
             double sigma_hit,
             double max_occ_dist,
             double z_reflector);
  ...
  public: const pf_vector_t& GetLaserPose() const { return laser_pose; }
  ...
  private: static double LikelihoodFieldModel(AMCLLaserData *data, 
                                              pf_sample_set_t* set);
  // Determine the probability for the given pose - with reflectors
  private: static double LikelihoodFieldModelReflectors(AMCLLaserData * data,
                         pf_sample_set_t* set);
  ...
  //
  // Landmarks
  private: const std::vector<pf_vector_t>* landmarks;
  private: double reflector_radius;
};
