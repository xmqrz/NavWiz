...
private:
  ...
  void landmarksReceived(const geometry_msgs::PoseArrayConstPtr& msg);
  void mapLayoutReceived(const std_msgs::String& msg)
  {
    map_layout_ = msg.data;
  }
  void handleMatchReflector(const std_msgs::Bool& msg)
  {
    reflector_matcher_.trigger(msg.data);
  }
  void handleTaskRunnerInitialized(const std_msgs::Bool& msg)
  {
    map_score_.updateEnable(msg.data);
  }

  void mapReceived(const nav_msgs::OccupancyGridConstPtr& msg);
  void handleMapMessage(const nav_msgs::OccupancyGrid& msg);
  void freeMapDependentMemory();
  void decodePng(nav_msgs::OccupancyGrid& map_msg);
  ...
  std::vector<pf_vector_t> map_landmarks_;
  std::string map_layout_;

  std::vector<boost::shared_ptr<message_filters::Subscriber<sensor_msgs::LaserScan> > > laser_scan_subs_;
  std::vector<boost::shared_ptr<tf2_ros::MessageFilter<sensor_msgs::LaserScan> > > laser_scan_filters_;
  ...
  ros::Publisher reflector_matched_pub_;
  ros::ServiceServer global_loc_srv_;
  ros::ServiceServer nomotion_update_srv_; //to let amcl update samples without requiring motion
  ros::ServiceServer set_map_srv_;
  ros::Subscriber map_sub_;
  ros::Subscriber landmarks_sub_;
  ros::Subscriber map_layout_sub_;
  ros::Subscriber match_reflector_sub_;
  ros::Subscriber task_runner_initialized_sub_;
  ...
  double z_hit_, z_short_, z_max_, z_rand_, z_reflector_, sigma_hit_, lambda_short_;
  ...
  double intensity_threshold_, far_intensity_threshold_, far_range_threshold_, reflector_min_width_, reflector_max_width_, reflector_radius_;
...
void sigintHandler(int sig)
{
  ...
  // Check master to avoid stalling at setParam.
  if (amcl_node_ptr && ros::master::check())
  {
...
AmclNode::AmclNode() :
  ...
  resample_count_(0),
  odom_(),
  laser_(0, NULL, 0, &map_landmarks_),
  inlier_(map_landmarks_),
  reflector_matcher_(map_landmarks_),
  map_score_(map_layout_),
  private_nh_("~"),
  ...
  first_map_received_(false)
{
  ...
  reflectors_pub_ = nh_.advertise<geometry_msgs::PoseArray>("reflectors", 2, true);
  reflector_matched_pub_ = nh_.advertise<std_msgs::Bool>("reflector_matched", 1, true);
  ...
  // Obtain scan topics from parameter
  std::string topics("scan");
  private_nh_.getParam("scan_topics", topics);

  std::istringstream iss(topics);
  std::string topic;
  while (iss >> topic)
  {
    laser_scan_subs_.push_back(
      boost::make_shared<message_filters::Subscriber<sensor_msgs::LaserScan> >(
        boost::ref(nh_), topic, 1));
  }

  initial_pose_sub_ = nh_.subscribe("initialpose", 2, &AmclNode::initialPoseReceived, this);
  landmarks_sub_ = nh_.subscribe("map_landmarks", 1, &AmclNode::landmarksReceived, this);
  map_layout_sub_ = nh_.subscribe("map_layout", 1, &AmclNode::mapLayoutReceived, this);
  match_reflector_sub_ = nh_.subscribe("match_reflector", 1, &AmclNode::handleMatchReflector, this);
  task_runner_initialized_sub_ = nh_.subscribe("/agv05_executor/task_runner_initialized", 1, &AmclNode::handleTaskRunnerInitialized, this);
  ...
  dsrv_.setCallback(boost::bind(&AmclNode::reconfigureCB, this, _1, _2));
  ...
  diagnosic_updater_.add("ICP", &icp_, &SnapMapICP::icpDiagnostics);
  diagnosic_updater_.add("Reflector matching", &reflector_matcher_, &ReflectorMatcher::diagnostics);
  diagnosic_updater_.add("Map Quality Score", &map_score_, &MapScore::diagnostics);
}
...
void AmclNode::reconfigureCB(AMCLConfig &config, uint32_t level)
{
  ...
  if (config.load_profile_)
  {

    if (config.load_profile_ == 1)
    {
      config.update_min_d = 0.025;
      config.update_min_a = 0.04;
      config.laser_z_hit = 0.5;
      config.laser_z_rand = 0.5;
      config.odom_model_type = "omni-corrected";
      config.odom_alpha1 = 0.8;
      config.odom_alpha2 = 0.8;
      config.odom_alpha3 = 0.8;
      config.odom_alpha4 = 0.8;
      config.odom_alpha5 = 0.8;
    }
    else if (config.load_profile_ == 2)
    {
      config.update_min_d = 0.025;
      config.update_min_a = 0.04;
      config.laser_z_hit = 0.8;
      config.laser_z_rand = 0.2;
      config.odom_model_type = "omni-corrected";
      config.odom_alpha1 = 0.8;
      config.odom_alpha2 = 0.8;
      config.odom_alpha3 = 0.8;
    ...
    else if (config.load_profile_ == 3)
    {
      config.update_min_d = 0.10;
      config.update_min_a = 0.07;
      config.laser_z_hit = 0.8;
      config.laser_z_rand = 0.2;
      config.odom_model_type = "diff-corrected";
      config.odom_alpha1 = 0.24;
      config.odom_alpha2 = 0.24;
      config.odom_alpha3 = 0.24;
      config.odom_alpha4 = 0.24;
      config.odom_alpha5 = 0.24;
    }

    config.min_particles = 500;
    config.max_particles = 2000;
    config.kld_err = 0.05;
    config.kld_z = 0.99;
    config.resample_interval = 1;
    config.selective_resampling = false;
    config.recovery_alpha_slow = 0.0;
    config.recovery_alpha_fast = 0.0;
    config.do_beamskip = false;
    config.tf_broadcast = true;
    config.force_update_after_initialpose = true;
    config.force_update_after_set_map = true;
    config.save_pose_rate = -1.0;
    config.laser_min_range = -1.0;
    config.laser_max_range = -1.0;
    config.laser_max_beams = 250;
    config.laser_z_reflector = 3.5;
    config.laser_sigma_hit = 0.1;
    config.laser_likelihood_max_dist = 4.0;
    config.laser_model_type = "likelihood_field_reflectors";

    config.match_rate = 0.0;  // disable Reflector Matching
    config.scan_rate = 0.0;  // disable ICP

    config.load_profile_ = 0;  // done
  }
  ...
  selective_resampling_ = config.selective_resampling;
  ...
  z_reflector_ = config.laser_z_reflector;
  ...
  else if (config.laser_model_type == "likelihood_field_reflectors")
    laser_model_type_ = LASER_MODEL_LIKELIHOOD_FIELD_REFLECTORS;
  ...
  beam_skip_error_threshold_ = config.beam_skip_error_threshold;

  // reflector
  intensity_threshold_ = config.reflector_intensity_threshold;
  far_intensity_threshold_ = config.reflector_far_intensity_threshold;
  far_range_threshold_ = config.reflector_far_range_threshold;
  reflector_min_width_ = config.reflector_min_width;
  reflector_max_width_ = config.reflector_max_width;
  reflector_radius_ = config.reflector_radius;
  ...
  // ICP
  icp_.reconfigure(base_frame_id_, global_frame_id_, config);

  // Reflector Matcher
  reflector_matcher_.reconfigure(base_frame_id_, global_frame_id_, config);

  // Map Score
  map_score_.reconfigure(config);

  // Inlier
  inlier_.reconfigure(config);

  // Laser scan message filter
  laser_scan_filters_.clear();
  for (int i = 0, n = laser_scan_subs_.size(); i < n; ++i)
  {
    laser_scan_filters_.push_back(
      boost::shared_ptr<tf2_ros::MessageFilter<sensor_msgs::LaserScan> >(
        new tf2_ros::MessageFilter<sensor_msgs::LaserScan>(
          *laser_scan_subs_[i], tf_, odom_frame_id_, 1, nh_)));

    laser_scan_filters_.back()->registerCallback(boost::bind(&AmclNode::laserReceived, this, _1));
  }
  ...
  // Laser
  laser_ = AMCLLaser(max_beams_, map_, reflector_radius_, &map_landmarks_);
  ...
  else if (laser_model_type_ == LASER_MODEL_LIKELIHOOD_FIELD_REFLECTORS)
  {
    ROS_INFO("Initializing likelihood field model; this can take some time on large maps...");
    laser_.SetModelLikelihoodFieldReflectors(z_hit_, z_rand_, sigma_hit_,
        laser_likelihood_max_dist_, z_reflector_);
    ROS_INFO("Done initializing likelihood field model with reflectors.");
  }
}
...
AmclNode::checkLaserReceived(const ros::TimerEvent& event)
{
  ...
  {
    ROS_WARN("No laser scan received (and thus no pose updates have been published) for %f seconds.  Verify that data is being published on the scan topics.", d.toSec());
  }
}

void AmclNode::landmarksReceived(const geometry_msgs::PoseArrayConstPtr& msg)
{
  boost::recursive_mutex::scoped_lock cfl(configuration_mutex_);

  geometry_msgs::Pose p;

  pf_vector_t landmark;
  map_landmarks_.clear();

  for (int i = 0; i < msg->poses.size(); i++)
  {
    landmark.v[0] = msg->poses[i].position.x;
    landmark.v[1] = msg->poses[i].position.y;
    landmark.v[2] = 0.0;

    map_landmarks_.push_back(landmark);
  }
}
...
void AmclNode::handleMapMessage(const nav_msgs::OccupancyGrid& msg)
{
  ...
  map_score_.updateMap(msg);
  decodePng(const_cast<nav_msgs::OccupancyGrid&>(msg));
  map_ = convertMap(msg);
  inlier_.updateMap(map_);
  icp_.updateMap(msg);
...
void AmclNode::decodePng(nav_msgs::OccupancyGrid& map_msg)
{
  const int8_t PNG_MAGIC_BYTES[8] = {-0x77, 'P', 'N', 'G', '\r', '\n', 0x1A, '\n'};
  if (map_msg.data.size() > 8 &&
      std::equal(map_msg.data.begin(), map_msg.data.begin() + 8, PNG_MAGIC_BYTES))
  {
    std::istrstream buf(reinterpret_cast<char*>(map_msg.data.data()), map_msg.data.size());
...
void AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  // Preprocess laser scan to identify reflectors
  std::vector<int> reflectors;
  if (laser_model_type_ == LASER_MODEL_LIKELIHOOD_FIELD_REFLECTORS)
  {
    int start = -1, end;

    for (int i = 0, n = laser_scan->intensities.size(); i <= n; i++)
    {
      if (i < n)
      {
        double intensity_threshold = (laser_scan->ranges[i] < far_range_threshold_) ?
                                     intensity_threshold_ : far_intensity_threshold_;
        if (laser_scan->intensities[i] >= intensity_threshold)
        {
          if (start < 0)
          {
            start = i;
      ...
      if (start >= 0)
      {
        double dth = laser_scan->angle_increment * (end - start);
        double dx = laser_scan->ranges[end] * cos(dth) - laser_scan->ranges[start];
        double dy = laser_scan->ranges[end] * sin(dth);
        double d = hypot(dx, dy);  // length of the reflective tape (~0.1m)

        //double d_interpolate = d * (i - start) / (end - start);
        //if (d_interpolate >= reflector_max_width_) {
        if (d > reflector_min_width_ && d < reflector_max_width_)
        {
          int sum = start + end;
          int idx = (sum >> 1);  // (start + end) / 2
          if (sum & 1)
          {
            if (laser_scan->ranges[idx] > laser_scan->ranges[idx + 1])
            {
              idx++;
            }
          }
  ...
  geometry_msgs::PoseArray reflectors_msg;
  reflectors_msg.header.stamp = laser_scan->header.stamp;
  reflectors_msg.header.frame_id = laser_scan_frame_id;
  for (int i = 0, n = reflectors.size(); i < n; i++)
  {
    int idx = reflectors[i];
    double range = laser_scan->ranges[idx] + reflector_radius_;
    double theta = laser_scan->angle_min + laser_scan->angle_increment * idx;

    geometry_msgs::Pose p;
    p.position.x = range * cos(theta);
    p.position.y = range * sin(theta);
    p.position.z = 0;
    p.orientation.w = 1.0;
    reflectors_msg.poses.push_back(p);
  }
  ...
  if (lasers_update_[laser_index])
  {
    ...
    if (!m_force_update && particlecloud_pub_.getNumSubscribers() > 0)
    {
  ...
  if (resampled || force_publication)
  {
    ...
    if (max_weight > 0.0)
    {
      ...
      // compute and update map quality score
      map_score_.update(last_published_pose.pose.pose, laser_scan_frame_id,
                        lasers_[laser_index].GetLaserPose(), ldata, inlier_);
    }
    else
    {
      ROS_ERROR("No pose!");
  ...
  geometry_msgs::PoseWithCovarianceStamped corrected_pose;
  if (icp_.processLaser(*laser_scan, tf_, inlier_, corrected_pose))
  {
    handleInitialPoseMessage(corrected_pose);
    last_published_pose = corrected_pose;
  ...
  if (reflector_matcher_.matchReflector(reflectors_msg, ldata, tf_, inlier_, matched_pose))
  {
    handleInitialPoseMessage(matched_pose);
    last_published_pose = matched_pose;

    std_msgs::Bool msg;
    msg.data = true;
    reflector_matched_pub_.publish(msg);
...
/* // Geometrical feature (Straight line or 90 degree corner) diagnostics
void
AmclNode::featureDiagnostics(diagnostic_updater::DiagnosticStatusWrapper& diagnostic_status)
{
  diagnostic_status.add("inlier_count", inlier_.size());
  diagnostic_status.add("min_inlier_count", min_inlier_count_);
  if (inlier_.size() < min_inlier_count_)
  {
    diagnostic_status.summary(diagnostic_msgs::DiagnosticStatus::WARN, "Too few features");
  }
  else
  {
    diagnostic_status.summary(diagnostic_msgs::DiagnosticStatus::OK, "OK");
...
void
AmclNode::mapQualityDiagnostics(diagnostic_updater::DiagnosticStatusWrapper& diagnostic_status)
{
  double score = map_score_.score();
  diagnostic_status.add("map_score", score);
  diagnostic_status.add("min_map_score", min_map_score_);
  if (score < min_map_score_)
  {
    diagnostic_status.summary(diagnostic_msgs::DiagnosticStatus::WARN, "Too low");
  }
  else
  {
    diagnostic_status.summary(diagnostic_msgs::DiagnosticStatus::OK, "OK");
