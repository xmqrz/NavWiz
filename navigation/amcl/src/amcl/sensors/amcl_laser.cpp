...
////////////////////////////////////////////////////////////////////////////////
// Default constructor
AMCLLaser::AMCLLaser(size_t max_beams, map_t* map, double reflector_radius,
                     const std::vector<pf_vector_t>* landmarks) : AMCLSensor(),
						     ...
						     temp_obs(NULL)
{
  ...
  this->reflector_radius = reflector_radius;
...
void
AMCLLaser::SetModelLikelihoodFieldReflectors(double z_hit,
               double z_rand,
               double sigma_hit,
               double max_occ_dist,
               double z_reflector)
{
  this->model_type = LASER_MODEL_LIKELIHOOD_FIELD_REFLECTORS;
  this->z_hit = z_hit;
  this->z_rand = z_rand;
  this->sigma_hit = sigma_hit;
  this->z_reflector = z_reflector;
  map_update_cspace(this->map, max_occ_dist);
}
...
bool AMCLLaser::UpdateSensor(pf_t *pf, AMCLSensorData *data)
{
  ...
  else if(this->model_type == LASER_MODEL_LIKELIHOOD_FIELD_REFLECTORS)
    pf_update_sensor(pf, (pf_sensor_model_fn_t) LikelihoodFieldModelReflectors, data);
  else
    pf_update_sensor(pf, (pf_sensor_model_fn_t) BeamModel, data);
...
double AMCLLaser::LikelihoodFieldModel(AMCLLaserData *data, pf_sample_set_t* set)
{
  ...
  int step = ceil((data->range_count) / static_cast<double>(self->max_beams)); 
  ...
  for (int j = 0; j < set->sample_count; j++)
  {
    ...
    for (int i = 0; i < data->range_count; i += step)
    {
      ...
      {
        double z = self->map->cells[MAP_INDEX(self->map,mi,mj)].occ_dist;
        // Gaussian model
...
double AMCLLaser::LikelihoodFieldModelReflectors(AMCLLaserData *data, pf_sample_set_t* set)
{
  AMCLLaser *self = (AMCLLaser*) data->sensor;
  double total_weight = 0.0;
  ...
  // second part: reflectors
  double max_occ_dist_sq = self->map->max_occ_dist * self->map->max_occ_dist;

  for (int j = 0; j < set->sample_count; j++)
  {
    std::vector<std::vector<double> > landmarks_dists_sq(data->reflectors.size());
    std::vector<int> landmarks_hits(self->landmarks->size(), 0);
    ...
    for (int i = 0; i < data->reflectors.size(); i++)
    {
      std::vector<double>& dists_sq = landmarks_dists_sq[i];
      dists_sq.resize(self->landmarks->size());

      int idx = data->reflectors[i];
      double obs_range = data->ranges[idx][0] + self->reflector_radius;
      double obs_bearing = data->ranges[idx][1];
      ...
      for (int k = 0; k < self->landmarks->size(); k++)
      {
        const pf_vector_t& landmark = (*self->landmarks)[k];
        pf_vector_t v = pf_vector_sub(hit, landmark);
        double dist_sq = v.v[0] * v.v[0] + v.v[1] * v.v[1];
        dists_sq[k] = dist_sq;
        if (dist_sq < max_occ_dist_sq)
        {
          landmarks_hits[k] += 1;
        }
      }
    ...
    for (int i = 0; i < data->reflectors.size(); i++)
    {
      ...
      double total_pz_weight = 0.0;
      double total_pz = 0.0;

      for (int k = 0; k < self->landmarks->size(); k++)
      {
        double dist_sq = dists_sq[k];
        if (dist_sq < max_occ_dist_sq)
        {
          double weight = 1.0 / landmarks_hits[k];
          double pz = self->z_reflector * exp(-dist_sq / z_hit_denom) * weight;

          total_pz_weight += weight;
          total_pz += pz;
        }
      }
      ...
      if (total_pz_weight)
      {
        double pz = total_pz / total_pz_weight;
        p += pz * pz;
      }
    }
...
double AMCLLaser::LikelihoodFieldModelProb(AMCLLaserData *data, pf_sample_set_t* set)
{
  ...
  for (int j = 0; j < set->sample_count; j++)
  {
    ...
    for (int i = 0, beam_ind = 0; i < data->range_count; i += step, beam_ind++)
    {
      double obs_range = data->ranges[i][0];
      double obs_bearing = data->ranges[i][1];
