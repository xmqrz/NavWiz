<script>
  ...
  function download(monthInteger) {
    ...
    agvActivities.downloadAgvActivities(payload).catch((e) => {
      console.error(e);
    });
  }
</script>
      ...
      <thead>
        <tr>
          <th class="!p-2 text-center">
            {#if data.previous}
              <a class="float-left" href="/config/agv-activities/downloads/{data.previous}">
                <i class="fa fa-lg fa-arrow-circle-left"></i>
              </a>
            {/if}
            {dy(data.current)}
            {#if data.next}
              <a class="float-right" href="/config/agv-activities/downloads/{data.next}">
