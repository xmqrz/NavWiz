<script>
  ...
  function save(overwrite = false) {
    let payload = Object.assign(editor.stage(), {
      modified: data.tTrigger.modified,
      overwrite: overwrite
    });
    maps
      .updateTTrigger(payload)
      .then(() => {
        toastStore.trigger({
          message: maps.successUpdateTTriggerMsg(),
          timeout: 3000,
          hoverable: true
        });
        dirty = false;
...
<ConfigLayout title="Edit map transition trigger" back={['Maps', maps.listUrl()]}>
  <MapParamEditorTab curMode="tTrigger" />
  {#if isOverwrite}
      ...
      <aside class="alert variant-filled-error">
        ...
        <div class="alert-message" data-toc-ignore="">
          ...
          <p>
            ...
            <a href="${maps.tTriggerUrl()}" target="_blank" class="font-bold">changed</a>
