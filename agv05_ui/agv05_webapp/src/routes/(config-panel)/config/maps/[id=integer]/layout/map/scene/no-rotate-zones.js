...
export default function (viz, scene) {
  ...
  var points = noRotateZones.append('g').attr('class', 'zone-active');
  var construct = noRotateZones
    .append('circle')
  ...
  function dragstarted(event, d) {
    ...
    noRotateZones.classed('drag-active', true);
  ...
  function dragended() {
    if (constructPoints.valid) {
      viz.models.push('dragNoRotateZoneCorner');
      activeZone.points = constructPoints.slice();
      constructPoints.valid = false;
      constructPoints.prev = undefined;
      viz.models.updateNoRotateZone(activeZone);
    } else {
    ...
    noRotateZones.classed('drag-active', false);
  ...
  function dragAllEnded() {
    if (constructPoints.valid) {
      viz.models.push('dragNoRotateZone');
      activeZone.points = constructPoints.slice();
      constructPoints.valid = false;
      viz.models.updateNoRotateZone(activeZone);
    } else {
  ...
  var remove = deleteMenu.find('.remove').on('click', function () {
    ...
    if (newPoints) {
      activeCoord = -1;
      constructPoints = newPoints;
      viz.models.push('removeNoRotateZoneCorner');
      activeZone.points = constructPoints.slice();
      viz.models.updateNoRotateZone(activeZone);
    }
    contextMenu.hide();
  ...
  function modelsUpdated() {
    ...
    if (activeObject instanceof NoRotateZone) {
  ...
  function enableEvents(enable) {
    if (!arguments.length || enable) {
      ...
      noRotateZones.classed('active', true);
    } else {
      noRotateZones.classed('active', false);
    }
  }
