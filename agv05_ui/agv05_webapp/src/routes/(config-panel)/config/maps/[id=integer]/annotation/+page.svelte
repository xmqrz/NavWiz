<script>
  ...
  import MapAnnotationEditor from './map_annotation/MapAnnotationEditor.svelte';
  import MapXAnnotationEditor from './mapx_annotation/MapXAnnotationEditor.svelte';
  ...
  function save(overwrite = false) {
    ...
    var payload = Object.assign(viz.editor.stage(), {
      modified: data.annotation.modified,
      overwrite: overwrite
    });

    maps
      .updateAnnotation(data.annotation.id, payload)
      .then((d) => {
        toastStore.trigger({
...
<ConfigLayout title={`Map "${data.annotation.name}"`} back={['Maps', maps.listUrl()]}>
  <MapEditorTab mapID={data.annotation.id} curMode="annotation" />
  {#if isOverwrite}
      ...
      <aside class="alert variant-filled-error">
        ...
        <div class="alert-message" data-toc-ignore="">
          ...
          <p>
            ...
            <a
              href="${maps.annotationUrl(data.annotation.id)}"
              target="_blank"
              class="font-bold">
              changed
            </a>
