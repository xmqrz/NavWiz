<script>
  ...
  import TaskTemplateEditorTab from '../components/TaskTemplateEditorTab.svelte';
  ...
  function save(overwrite = false) {
    let payload = Object.assign(editor.stage(), {
      modified: data.globalParam.modified,
      overwrite: overwrite
    });
    taskTemplates
      .updateGlobalParam(payload)
      .then(() => {
        toastStore.trigger({
          message: taskTemplates.successUpdateGlobalParam(),
          timeout: 3000,
          hoverable: true
        });
        dirty = false;
...
<ConfigLayout
  title="Edit task template global parameters"
  back={['Task Templates', taskTemplates.listUrl()]}>
  <TaskTemplateEditorTab curMode="global-param" />
  {#if isOverwrite}
      ...
      <aside class="alert variant-filled-error">
        ...
        <div class="alert-message" data-toc-ignore="">
          ...
          <p>
            ...
            <a href="${taskTemplates.globalParamUrl()}" target="_blank" class="font-bold">
              changed
            </a>
