<script>
  ...
  function save(overwrite = false) {
    let payload = Object.assign(editor.stage(), {
      modified: data.variable.modified,
      overwrite: overwrite
    });
    taskTemplates
      ...
      .then(() => {
        toastStore.trigger({
          message: taskTemplates.successUpdateVariable(),
          timeout: 3000,
          hoverable: true
        });
        dirty = false;
...
<ConfigLayout
  title="Edit task template variables"
  back={['Task Templates', taskTemplates.listUrl()]}>
  <TaskTemplateEditorTab curMode="variable" />
  {#if isOverwrite}
      ...
      <aside class="alert variant-filled-error">
        ...
        <div class="alert-message" data-toc-ignore="">
          ...
          <p>
            ...
            <a href="${taskTemplates.variableUrl()}" target="_blank" class="font-bold">
              changed
            </a>
